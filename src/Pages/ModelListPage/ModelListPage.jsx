...
const ModelListPage = () => {
  ...
  const toggleModelSelection = (model) => {
    setSelectedModels((prev) => {
      if (prev.some((m) => m._id === model._id)) {
        return prev.filter((m) => m._id !== model._id); // Remove
      } else if (prev.length < 4) {
        return [...prev, model]; // Add up to 4
      } else {
  ...
  const handleContinue = () => {
    ...
    const selectedImages = selectedModels.map((model) =>
      Array.isArray(model.imageUrl) ? model.imageUrl[0] : model.imageUrl
    );
  
    navigate("/virtualtryon", { state: { selectedImages } });
  };
